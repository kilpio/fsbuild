...
withCredentials([usernameColonPassword(credentialsId: 'GITHUB_SSH_CREDENTIALS_ID', variable: 'USERPASS')]) {
    def userpass_=string(USERPASS)
    sh "echo ${userpass_} > /tmp/USERPASS_"
    sh "cat /tmp/USERPASS_"
      }
...
echo "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    checkout([$class                           : 'GitSCM', branches: [ [name: "*/${MASTER_BRANCH}"], [name: "*/${branch}"]],
            ...
            userRemoteConfigs                : [[credentialsId: "alkilpio/${GITHUB_SSH_CREDENTIALS_ID}", url: "git@github.com:${GIT_REPO_OWNER}/${repositoryName}.git"]],
